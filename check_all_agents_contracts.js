...
async function run() {
    ...
    console.log(`Checking all registered agent wallets for contract code...`);
    for (const log of aLogs) {
        ...
        const codeRes = await (await fetch(RPC_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ jsonrpc: "2.0", method: "eth_getCode", params: [wallet, "latest"], id: 1 })
        })).json();

        if (codeRes.result && codeRes.result !== "0x") {
            console.log(`Agent ID ${id} (${wallet}) IS A SMART CONTRACT.`);
        }
    }
}
